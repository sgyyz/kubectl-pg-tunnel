...
install_plugin() {
    ...
    # Copy plugin
    if ! cp "${PLUGIN_NAME}" "${plugin_path}"; then
        print_error "Failed to copy plugin to ${plugin_path}"
        exit 1
    fi
...
setup_config() {
    ...
    if [[ ! -f "${CONFIG_FILE}" ]]; then
        if [[ -f "config/config.yaml.example" ]]; then
            ...
            print_warning "Example config not found, skipping config creation"
            print_info "You'll need to create ${CONFIG_FILE} manually"
        fi
    else
        print_info "Config file already exists: ${CONFIG_FILE}"
        print_info "To see the latest example, check: config/config.yaml.example"
    fi
}
...
print_instructions() {
    ...
    echo "   - PROFILE_CONTEXT_*: Your kubectl contexts"
    echo "   - DB_HOST_*: Your database hostnames"
    echo ""
    ...
    echo "   kubectl pg-tunnel -p staging your_database_alias"
    echo ""
...
main() {
    ...
    if [[ ! -f "${PLUGIN_NAME}" ]]; then
        print_error "Plugin file not found: ${PLUGIN_NAME}"
        echo ""
    ...
    # Install plugin
    install_plugin "${install_dir}"

    # Setup configuration
    setup_config

    # Verify installation (non-fatal if verification fails)
    # shellcheck disable=SC2310
